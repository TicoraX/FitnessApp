...
function createProgram(gl: WebGL2RenderingContext, shaderSources: [string, string], transformFeedbackVaryings?: string[], attribLocations?: Record<string, number>) {
...
function makeVertexArray(gl: WebGL2RenderingContext, bufLocNumElmPairs: Array<[WebGLBuffer | null, number, number]>, indices?: Uint16Array) {
...
class InfiniteGridMenu {
  ...
  constructor(canvas: HTMLCanvasElement, items: MenuItem[], onActiveItemChange: (index: number) => void, onMovementChange: (moving: boolean) => void, onInit?: ((menu: InfiniteGridMenu) => void) | null, scale = 1.0) {
    this.canvas = canvas;
    this.items = items || [];
...
const defaultItems: MenuItem[] = [
  {
    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512"><rect width="512" height="512" fill="%231a202c"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23a0aec0" font-family="sans-serif" font-size="32">FitTrack</text></svg>',
    title: 'FitTrack',
    description: 'Catálogo Nutricional'
  }
];
...
export default function InfiniteMenu({ items = [], scale = 1.0, onSelect }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      const activeList = items.length ? items : defaultItems;
      const itemIndex = index % activeList.length;
      setActiveItem(activeList[itemIndex]);
    };
